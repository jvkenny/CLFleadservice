...
/**
 * Lake Forest Lead Service Inventory - Resident Portal
 * -------------------------------------------------------------
 * Designed for clarity and ease of use for all residents
 * Features educational content and calm, accessible design
 */
...
const MOCK_LOCATIONS: Location[] = [
  {
    ...
    customerMaterial: "Lead",
    supplierMaterial: "Copper",
    letterSent: true,
    buildYear: 1952,
    status: "VERIFIED",
    lat: 42.1,
  ...
  },
  {
    ...
    customerMaterial: "Copper",
    supplierMaterial: "Lead",
    letterSent: false,
    buildYear: 1968,
    status: "ASSUMED",
    lat: 42.11,
  ...
  },
  {
    ...
    customerMaterial: "Unknown",
    supplierMaterial: "Unknown",
    letterSent: false,
    buildYear: 1945,
    status: "UNKNOWN",
    lat: 42.09,
...
export default function LsiViewer() {
  ...
  const [filterMaterial, setFilterMaterial] = useState("all"); // all, lead, unknown
  ...
  const stats = {
    ...
    lead: MOCK_LOCATIONS.filter(
      (loc) =>
        loc.customerMaterial === "Lead" || loc.supplierMaterial === "Lead"
    ).length,
    unknown: MOCK_LOCATIONS.filter(
      (loc) =>
        loc.customerMaterial === "Unknown" || loc.supplierMaterial === "Unknown"
    ).length,
  };
  ...
  const filteredLocations = MOCK_LOCATIONS.filter((loc) => {
    ...
    if (filterMaterial === "lead" && loc.customerMaterial !== "Lead" && loc.supplierMaterial !== "Lead") {
      return false;
    }
    if (filterMaterial === "unknown" && loc.customerMaterial !== "Unknown" && loc.supplierMaterial !== "Unknown") {
      return false;
    }
    return true;
  });
          ...
          {loading ? (
            <div className="w-full h-full flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-xl" style={{ color: LF_GREEN }}>Loading map...</p>
              </div>
            </div>
          ) : (
            <div ref={mapDiv} className="w-full h-full flex items-center justify-center p-8" style={{ backgroundColor: LF_MUTED_GREEN }}>
              <div className="text-center p-12 bg-white/95 backdrop-blur rounded-2xl shadow-xl max-w-2xl border" style={{ borderColor: LF_MEDIUM_GREEN }}>
                <MapPin className="w-20 h-20 mx-auto mb-6" style={{ color: LF_GREEN }} />
                <h3 className="text-2xl font-semibold mb-4" style={{ color: LF_GREEN }}>Interactive Map View</h3>
                <p className="text-lg mb-8 leading-relaxed" style={{ color: LF_DARK_GREEN }}>
                  In the live version, you'll see an interactive map of Lake Forest showing service line 
                  locations. Use the search bar above to find your address.
                </p>
                <div className="grid grid-cols-3 gap-4 mt-8">
                  {filteredLocations.slice(0, 6).map((loc) => (
                    <button
                      ...
                      className="p-6 hover:shadow-lg rounded-xl border-2 transition-all"
                      style={{
                        borderColor: selected?.id === loc.id ? LF_GREEN : LF_MUTED_GREEN,
                        backgroundColor: selected?.id === loc.id ? LF_LIGHT_GREEN : 'white'
                      }}
                    >
                      <MapPin className="w-8 h-8 mx-auto mb-2" style={{ color: LF_GREEN }} />
                      <div className="text-sm font-medium" style={{ color: LF_DARK_GREEN }}>{loc.address}</div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white border-2 border-stone-200 rounded-xl p-5">
                    <div className="flex items-start gap-3 mb-3">
                      <Droplet className="w-6 h-6 text-stone-600 flex-shrink-0 mt-1" />
                      <div className="flex-1">
